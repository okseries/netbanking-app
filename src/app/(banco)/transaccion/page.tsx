        ...
        <div className="bg-white rounded-lg shadow-sm border border-slate-200">
          <div className="p-6">
            <div className="flex items-center gap-2 mb-6">
              <IoSend className="text-lg text-blue-600" />
              <div>
                <h1 className="text-lg font-medium text-slate-800">Nueva Transferencia</h1>
                <p className="text-sm text-slate-500">Envía dinero de forma segura entre cuentas</p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full p-3 text-sm border border-slate-300 rounded-lg bg-white text-slate-700 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                >
                  <option value="">Selecciona tu cuenta</option>
                  {productos.map((producto) => (
                    <option key={producto.id} value={producto.id}>
                      {producto.tipo === 0 ? 'Ahorros' : 'Corriente'} - {producto.referencia} ({formatCurrency(producto.balance)})
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 p-3 text-sm border border-slate-300 rounded-lg bg-white text-slate-700 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                    value={destino}
                    onChange={(e) => setDestino(e.target.value)}
                  />
                  <button 
                    onClick={validarOrigen} 
                    className="px-3 py-3 bg-blue-600 hover:bg-blue-700 text-white text-xs font-medium rounded-lg transition-colors"
                  >
                    Verificar
                  </button>
                </div>
              ...
              {detalleCuenta && (
                <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                  <div className="flex items-center gap-1 mb-1">
                    <IoCheckmarkCircle className="text-sm text-green-600" />
                    <span className="text-xs font-medium text-green-800">Cuenta verificada</span>
                  </div>
              ...
              <div className="flex gap-2 pt-3">
                <button
                  onClick={resetForm}
                  className="flex-1 py-2 px-3 border border-slate-300 text-slate-700 text-sm font-medium rounded-lg hover:bg-slate-50 transition-colors"
                >
                  Cancelar
                </button>
                <button
                  ...
                  className="flex-1 py-2 px-3 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300 disabled:cursor-not-allowed text-white text-sm font-medium rounded-lg transition-colors"
                >
                  Transferir
                </button>
              </div>
